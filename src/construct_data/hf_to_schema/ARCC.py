...
def transform_arcc(test_instance: dict):
    ...
    schema[SCHEMA_KEYS.DATA_SET.value] = "ARCC"
...
class ARCC:
    """
    This Class holds the ARC-Challenge dataset post transformation into the schema required
    """

    def __init__(self):
        """
        Initialize ARCC instance
        """
        super().__init__()
        # load the dataset
        ...
        self.intermediate_representation = self.dataset.map(
            transform_arcc, remove_columns=self.dataset.column_names
        )
