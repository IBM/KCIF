...
def sort_options_to_create_string(input_instance: dict):
    """This function applies the sort_options_to_create_string instruction on
    ...
    input_instance[SCHEMA_KEYS.INSTRUCTION_ID.value] = (
        sort_options_to_create_string_schema["instruction_id"]
    )
    new_instruction = sort_options_to_create_string_schema["instruction_text"][
        r.randint(0, len(sort_options_to_create_string_schema["instruction_text"]) - 1)
    ]
    ...
    input_instance[SCHEMA_KEYS.INSTRUCTION_OUTPUT.value].append(
        apply_instruction(input_instance[SCHEMA_KEYS.CANDIDATE_ANSWER_SET.value])
    )
